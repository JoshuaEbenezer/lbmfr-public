...
filenames = glob.glob(os.path.join('./distorted_vids_dmos_zscores','*.csv'))
...
for subject_csv in filenames:
    ...
    for index,v in enumerate(video_names):
        score = subject_df['raw_dmos_zscore'].iloc[index]
...
score_df = pd.DataFrame(list(zip(names,scores,content,codec,fr,res,bitrate)),columns=['video','dmos','content','codec','fr','res','bitrate'])
score_df.to_csv('./lbvfr_dmos.csv')
