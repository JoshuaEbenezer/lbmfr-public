...
def cut_video_and_copy(group,index):
    all_commands = []
    outfolder = '../all_cut_y4m_vids' 
    for vidname in group:
        content_name = vidname.split('_')[0]
        filenames = glob.glob(os.path.join('../../full_length_distorted_y4m',content_name+'_AVC_SFR_*'))
        print(content_name)
#        if(content_name!='EPLDay'):#and content_name!='EPLNight'):
#            continue
        for input_vid in filenames:
            ...
            fr = base.split('_')[-3]
            res_shorthand = base.split('_')[-2]
            resolution = expand_res_name(res_shorthand)

            width = int(resolution.split('x')[0])
            height = int(resolution.split('x')[1])

            fps = fps_from_content(content_name,fr)
