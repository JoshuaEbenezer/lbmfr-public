...
def single_vid_psnr(i):
    ...
    if(os.path.exists(ref_video)==False):
        return

    psnr_outname = os.path.join('./psnr_features_mint/',os.path.splitext(os.path.basename(dis_vid))[0]+'.log')
#    ssim_outname = os.path.join('./ssim_features_mint/',os.path.splitext(os.path.basename(dis_vid))[0]+'.log')
...
#    ssim_command = ' '.join(['/home/ubuntu/daala/tools/dump_ssim','-r','-y','-s',dis_vid,ref_video,'2>&1 >>',ssim_outname])
    #try:
#    subprocess.check_call(ssim_command,shell=True)
