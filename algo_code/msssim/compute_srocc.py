...
filenames = glob.glob(os.path.join('./msssim_features_PR','*.log'))
...
for infile in filenames:
    ...
    try:
        ...
        continue
    if('SFR' in vid_name):
        fr_file = os.path.join('./msssim_features_mint/',vid_name+'.log')
    else:
        fr_file = infile
    ...
    with open(fr_file) as f:
        ...
        y_msssim = float(line.split('Total:')[1][1:8])
        print(y_msssim)
        msssim.append(y_msssim)
    all_msssim.append(np.mean(msssim))
    all_dmos.append(dmos)

results(all_msssim,all_dmos)
