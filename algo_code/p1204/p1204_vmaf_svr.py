...
#    feature_folder= "/home/ubuntu/bitstream_mode3_p1204_3/features/p1204_etri_features"
    ...
    vmaf_feature_folder = '../vmaf/vmaf_features_PR'
    train_names = []
    val_names = [] 
    for i,vid in enumerate(video_names):
        if('SRC' in vid):
            continue
        content = vid.split('_')[0]
        featfile_name = vid+'.z'
        vmaf_featfile_name = vid+'.json'
        json_f = os.path.join(vmaf_feature_folder,vmaf_featfile_name)
...
#        vmaf_feature = np.asarray(feature_list,dtype=np.float32)
        try:
    ...
    #            print(json_data)
        ...
        feature = np.concatenate((feat_file['features'],vmaf_feature),dtype=np.float32)
